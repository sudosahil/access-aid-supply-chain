...
export const AssetDistribution = () => {
  ...
  const handleReturnAsset = (distributionId: string) => {
    setDistributions(distributions.map(dist => dist.id === distributionId ? {
      ...dist,
      status: 'returned',
      ...
      <Card>
        <CardHeader className="bg-slate-300">
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription className="text-slate-950">Manage asset distribution and tracking</CardDescription>
            </div>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={formData.itemId} onValueChange={value => setFormData({
                    ...formData,
                    itemId: value
                  })}>
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        {mockInventoryItems.map(item => <SelectItem key={item.id} value={item.id}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input id="assignedTo" value={formData.assignedTo} onChange={e => setFormData({
                      ...formData,
                      assignedTo: e.target.value
                    })} placeholder="e.g., Regional Office North" required />
                    </div>
                  </div>
                  <div>
                  ...
                  })} placeholder="e.g., Client loan program" required />
                  </div>
        ...
        <CardContent className="bg-slate-300">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="bg-slate-950">Item</TableHead>
                <TableHead className="bg-slate-950">Quantity</TableHead>
                <TableHead className="bg-slate-950">Assigned To</TableHead>
                <TableHead className="bg-slate-950">Purpose</TableHead>
                <TableHead className="bg-slate-950">Distribution Date</TableHead>
                <TableHead className="bg-slate-950">Status</TableHead>
                <TableHead className="bg-slate-950">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {distributions.map(distribution => <TableRow key={distribution.id}>
                  ...
                  <TableCell>
                    {distribution.status === 'active' && <Button variant="outline" size="sm" onClick={() => handleReturnAsset(distribution.id)}>
